...
try:
    ...
    import time
    import sqlite3
# Imports all the packages. I needed to pip install a few of them.

except:
     import_exception = """ Could not load some modules """
     print import_exception 
     sys.exit(3)
# Tells you if they don't load, exits system, reporting exit status 3.
...
# Setting up the script. It defines what the script does, and the input variables.
# -i is our input, which will go the the variable called input_files, as a string. There can be multiple arguments entered, it is necessary to have it, and there is a little help mesage, and no default.
# --db gets given the name sqlite_db, is a string, and should have ? arguments added, is required, and has a help message, and there is no default.
...
def parse_cazy_row(row, fields):
    ...
    """
    # There is a function called parse_cazy_row that requires two inputs - row, and fields.
...
def parse_cazy_and_insert(file_handle, family, family_num, db_cursor):
    ...
    sql = "INSERT INTO CAZY_2015_03_11 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
